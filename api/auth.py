...
async def create_user(*,session:Session = Depends(get_session), user: UserCreate):
    """Create a new user"""
    user_model = Users(username=user.username,useremail=user.useremail, password=password_context.hash(user.password))
    user_item = Users.model_validate(user_model)
